...
final class JsonDeserializer {
    ...
    private Node _deserialize(JsonParser p) {
        var stack = new ContainerFrameStack();
    ...
    private ContainerNode _deserializeContainerNoRecursion(JsonParser p, ContainerFrameStack stack, final ContainerNode root) throws JacksonException {
        ContainerFrame curr = new ContainerFrame(root);

        outer_loop:
        do {
            var currContainer = curr.container();
            switch (currContainer) {
                case ObjectNode currObject -> {
                    ...
                    while (true) {
                        ...
                        if (t == null) {
                            throw new StreamReadException(p, "Unexpected end-of-input");
                        }
                        switch (t) {
                            case START_OBJECT -> {
                                var n = new ObjectNode();
                                var old = currObject.put(propName, n);
                                if (old != null) {
                                    _handleDuplicateProperty(p, propName, currObject, old, n);
                                }
                                stack.push(curr);
                                curr = new ContainerFrame(n);
                                continue outer_loop;
                            }
                            case START_ARRAY -> {
                                var n = new ArrayNode();
                        ...
                        var old = currObject.put(propName, value);
                        if (old != null) {
